...
typed_state = TypedState(
    state, TrameState, state_encoder=CollectionEncoderDecoder([CustomEncoder()])
)
...
def on_inner_data_change(inner_data: Inner):
    ...
    print(f"Inner data changed: {as_dataclass(inner_data)}")
...
# TypedState provides a bind_change method to binds state changes to strongly typed callbacks
# Binding state changes can be done using a key / value approach.
typed_state.bind_change(typed_state.name.inner.text_value, on_text_change)

# It can also be done using a dict format.
...
@ctrl.add("reset_task")
def reset_task():
    # Dataclass values can be set from a dataclass instance using the from_dataclass method
    from_dataclass(typed_state.data.inner, Inner())
